...
dados <- dados %>%
  mutate(
    ...
    AGENTE_f = factor(case_when(
      ...
      TP_ACIDENT == "3" & ANI_ESCORP == "1" ~ "Escorpião - Tityus (Amarelo/Marrom)",
      TP_ACIDENT == "3" & ANI_ESCORP == "2" ~ "Escorpião - Outros",
...
g1 <- ggplot(freq_agente, aes(x = reorder(AGENTE_f, n), y = n, fill = TIPO_ANIMAL)) +
  ...
  geom_text(
    aes(label = ifelse(n > 1000000, 
                       paste0(format(round(n/1000000, 2), decimal.mark=","), " Mi"),
                       paste0(format(round(n/1000, 1), decimal.mark=","), " mil"))),
  ...
  scale_y_continuous(
    # Formata os números do eixo (0, 100 mil, 200 mil...)
    labels = label_number(scale = 0.001, suffix = " mil", big.mark = ".", decimal.mark = ","),
    # Expande o limite para o texto não cortar na direita
    expand = expansion(mult = c(0, 0.15)) 
  ) +
  
  # 6. Títulos e Tema
  labs(title = "Notificações por Agente Específico",
       subtitle = "Total de casos registrados no período analisado",
...
g2 <- ggplot(dados_atuais %>% filter(IDADE_NUM > 0 & IDADE_NUM < 95), aes(x = IDADE_NUM)) +
  ...
  scale_y_continuous(
    ...
    # suffix = " mil" -> Adiciona o texto no final
    labels = label_number(scale = 0.001, suffix = " mil", big.mark = ".", decimal.mark = ",")
  ) +
  scale_x_continuous(
...
g3 <- ggplot(grav_x_animal, aes(x = TIPO_ANIMAL, y = n, fill = GRAVIDADE_f)) +
...
g3_agrupado <- ggplot(grav_x_animal, aes(x = TIPO_ANIMAL, y = Percentual, fill = GRAVIDADE_f)) +
  ...
  labs(
    ...
    subtitle = "Proporção de casos Leves, Moderados e Graves em cada grupo",
    x = "Grupo Animal",
...
g_let <- ggplot(letalidade, aes(x = reorder(AGENTE_f, Letalidade_pct), y = Letalidade_pct)) +
...
g_let_cor <- ggplot(letalidade, aes(x = reorder(AGENTE_f, Letalidade_pct), y = Letalidade_pct)) +
  ...
  labs(
    ...
    subtitle = "Considerando apenas agentes com mais de 50 casos registrados",
